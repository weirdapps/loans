...
- `MAX_ITERATIONS = 1_000_000` guards the convergence loop in both `app.py` and `cli.py`; on breach, `app.py` re-renders the form with an error and `cli.py` exits with status 1
...
- **dependabot-auto-merge.yml**: auto-merges Dependabot patch, minor, and grouped updates; major bumps still require manual review.
