...
def calculate():
    ...
    try:
        ...
        return render_template(
            "index.html", error="Invalid input. Please enter valid numeric values."
        )

    if loan_amount <= 0:
        return render_template("index.html", error="Loan amount must be greater than 0.")
    if duration <= 0:
        return render_template("index.html", error="Duration must be greater than 0.")
    if annual_interest_rate <= 0:
        return render_template("index.html", error="Annual interest rate must be greater than 0.")
    if partner_rate < 0:
        return render_template("index.html", error="Commission rate must be 0 or greater.")
    ...
    while remaining_balance > 0.10:
        ...
        if iterations > MAX_ITERATIONS:
            return render_template(
                "index.html", error="Calculation did not converge. Please check your inputs."
            )
